...
if __name__ == '__main__':
  ...
  train_dataset_dict, test_dataset_dict = dataset.load_data(args.load)
  # エンコーダーの読み込み
  model = VanillaVAE(in_channels = in_channels, latent_dim=latent_dim)
  model_save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../VAE/output/model.pth')
  ...
  for key in train_dataset_dict.keys():
    ...
    for train_data in train_dataset:
      ...
      combined = np.concatenate((goal, velocity, neighbor, depth))
