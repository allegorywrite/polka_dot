...
class ActionType(Enum):
    ...
    VEL5D = "vel5d"         # Full velocity input (using PID control)
...
class BaseDotAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        if act in [ActionType.PID, ActionType.VEL, ActionType.TUN, ActionType.ONE_D_PID, ActionType.VEL5D]:
    ...
    def _preprocessAction(self,
        ...
        if self.ACT_TYPE == ActionType.TUN:
            ...
            rpm, _, _ = self.ctrl.computeControl(control_timestep=self.AGGR_PHY_STEPS*self.TIMESTEP, 
                                                 ...
                                                 target_rpy=np.array([0,0,state[9]]), # keep current yaw
                                                 target_vel=self.SPEED_LIMIT * np.abs(action[3]) * v_unit_vector, # target the desired velocity vector
                                                 target_rpy_rates=np.array([0,0,action[4]])
                                                 )
            return rpm
        else:
    ...
    def _observationSpace(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            ############################################################
            #### Observation vector ### Gx Gy Gz   Vx Vy Vz  Depth
            # obs_lower_bound = np.array([-1,-1,-1,  -1,-1,-1,  128x128])
            # obs_upper_bound = np.array([1,1,1,     1,1,1,  128x128])
            ############################################################
            state_lower_bound = np.array([-1,-1,-1,  -1,-1,-1])
            state_upper_bound = np.array([1,1,1,  1,1,1])
            return spaces.Dict({"state": spaces.Box(low=state_lower_bound,
