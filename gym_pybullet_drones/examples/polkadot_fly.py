...
def run(
    ...
    env = PolkadotAviary(drone_model=drone,
    ...
    action = {str(i): np.array([0,0,0,0,0]) for i in range(num_drones)}
    START = time.time()
    for i in range(0, int(duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            action = {str(i): np.array([1,0,0.05,10,1]) for i in range(num_drones)}
